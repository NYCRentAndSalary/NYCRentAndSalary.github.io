## Overall project checklist

### Overall requirements

* [ ] Website
* [ ] Explainer notebook hosted on website

### Website requirements

* [ ] Visualizations
* [ ] Data story
* [ ] Cohesive design

#### Our requirements/notes
* [ ] Don't express certainty, if the data doesn't support it
* [ ] Make sure, the text looks like, it was written by us - read through for AI formulations

### Notebook requirements

#### Motivation
* [ ] Explain dataset?
* [ ] Explain choice of this/these particular dataset(s)?
* [ ] What's the goal for the reader's experience?

#### Basic stats
* [ ] Explain data cleaning and preprocessing
* [ ] Explain dataset stats, key points/plots from exploratory data analysis concisely

#### Data Analysis
* [ ] Explain data analysis and explain what you've learned about the dataset
* [ ] If relevant, talk about ML

#### Genre
* [ ] Mention used tools from each of the 3 categories of Visual Narrative (Figure 7 in Segal and Heer). Why?
* [ ] Mention used tools from each of the 3 categories of Narrative Structure (Figure 7 in Segal and Heer). Why?

#### Visualizations
* [ ] Explain visualizations
* [ ] Explain why they're right for the story

#### Discussion
* [ ] What went well?
* [ ] What is still missing? What could be improved? Why?

#### Contributions and references
* [ ] Explain who did what
* [ ] Include references


### Ideas for plots
* [ ] Plot of change in average rent and salary - Per borough
    * [ ] Are there specific times of year, where rent spikes? TImes that are better to look for apartments?
    * [ ] When did rent increase so much, that it is no longer realistic to pay on a public salary?
* [ ] Distribution of apartment type per borough
* [ ] Heatmap of difference in average rent and income in each borough - Sofie
* [ ] Change in top 10(?) jobs, are there some jobtypes, that have a smaller/larger increase in salay? Do they have something in common? Correlation?
* [ ] Correlation between gentrification and rent increase (other factors)
* [ ] Look at policy changes, that might have affected rent and/or income
* [ ] 